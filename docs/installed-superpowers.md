# Installed Superpowers Index

Faithful inventory generated from `C:\Users\consa\.codex\superpowers\skills`.
...
| `using-git-worktrees` | `superpowers-skills/using-git-worktrees` | Use when starting feature work that needs isolation from current workspace or before executing implementation plans - creates isolated git worktrees with smart directory selection and safety verification |
| `using-superpowers` | `superpowers-skills/using-superpowers` | Use when starting any conversation - establishes how to find and use skills, requiring Skill tool invocation before ANY response including clarifying questions |
